...
/* ========== Legacy browser brīdinājums (tāds pats dizains kā uploaderam) ========== */
/* Chrome 60–69: redz brīdinājumu, bet rīks turpina strādāt. Vecākiem – arī.       */
/* Nekādas modernās sintakses: bez ||=, ?. vai arrow funkcijām.                     */
(function () {
  // minimālais “ērtais” līmenis (zemāk rādam brīdinājumu)
  var MIN_CHROME = 70; // chrome 60 skolai darbosies, bet parādām paziņojumu
  ...
  function detectMissingFeatures() {
    ...
    try { new URL('https://example.com'); } catch (e) { missing.push('URL'); }
  ...
  function isOldChromium(minMajor) {
    var ua = navigator.userAgent || '';
    var m  = ua.match(/Chrom(e|ium)\/(\d+)/);
    if (!m) return false;
    var ver = parseInt(m[2], 10);
    return ver < (minMajor || 70);
  }

  function showLegacyBrowserWarning(missingList) {
    // jau ir atvērts → nerādām divreiz
    ...
    // saliekam “trūkstošo iespēju” sarakstu (ja ir)
    var missingHTML = '';
    if (missingList && missingList.length) {
      missingHTML =
        '<div class="dropzone" style="cursor:default">' +
          '<div class="big">Trūkstošās iespējas:</div>' +
          '<small>' + missingList.join(', ') + '</small>' +
        '</div>' +
        '<hr class="divider">';
    }

    // karte ar to pašu vizuālo stilu
    wrap.innerHTML =
      '<div class="uploader-card" role="dialog" aria-labelledby="legacyWarnTitle" aria-describedby="legacyWarnText">' +
        '<h3 id="legacyWarnTitle">Jūsu pārlūks ir novecojis</h3>' +
        '<hr class="divider">' +
        '<p id="legacyWarnText">Dažas rīka funkcijas var nedarboties (piemēram, PDF importēšana, tiešsaistes karte vai gludas animācijas). ' +
        'Lai izmantotu visu funkcionalitāti, lūdzu, atjauniniet pārlūku.</p>' +
        missingHTML +
        '<div class="uploader-actions">' +
          '<button id="legacyUpdate" class="btn primary" type="button">Atjaunināt pārlūku</button>' +
          '<button id="legacyClose"  class="btn danger"  type="button">Turpināt</button>' +
        '</div>' +
      '</div>';

    document.body.appendChild(wrap);

    // “Atjaunināt pārlūku” – atveram jaunu cilni (vari nomainīt adresi uz savām norādēm)
    var upd = document.getElementById('legacyUpdate');
    if (upd) {
      upd.addEventListener('click', function () {
        // izvēlies, ko gribi rādīt skolai/lietotājiem:
        var url = 'https://browsehappy.com/';
        try { window.open(url, '_blank', 'noopener'); } catch (e) { location.href = url; }
      }, false);
    }

    // “Turpināt” vai klikšķis uz backdrop
  ...
  function shouldWarn() {
    // brīdinām, ja ļoti vecs Chrome/Chromium VAI ja pietrūkst ≥2 svarīgu feature’u
    var missing = detectMissingFeatures();
    var uaOld   = isOldChromium(MIN_CHROME);
    var show    = uaOld || (missing.length >= 2);
    return { show: show, missing: missing };
  }

  // palaidam, kad DOM gatavs (tas pats ritms kā citur projektā)
  onDomReady(function () {
    // rādi tikai 1x sesijā
    try {
      if (sessionStorage.getItem('legacyWarnShown') === '1') return;
    } catch (_e) {}

    var res = shouldWarn();
    if (res.show) {
      showLegacyBrowserWarning(res.missing);
